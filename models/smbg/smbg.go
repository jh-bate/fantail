...
//stream incoming data and create and then write Smbg values as JSON
func Stream(src io.Reader, dest io.Writer, deviceId, uploadId string) error {
	return Decode(src).Set(deviceId, uploadId).Encode(dest)
}
